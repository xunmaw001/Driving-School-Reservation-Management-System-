        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
		    ...
		    label="备注">
		     <template slot-scope="scope">
                ...
                label="操作">
                <template slot-scope="scope">
                                <el-button v-if="isAuth('yuyue','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                <el-button v-if="isAuth('yuyue','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yuyue','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('yuyue','取消预约') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="quxiaoyuyueCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'取消预约':'' }}</el-button>
                <el-button v-if="isAuth('yuyue','取消预约') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="quxiaoyuyueCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'取消预约':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yuyue','取消预约') && contents.tableBtnIcon == 0" type="success" size="mini" @click="quxiaoyuyueCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'取消预约':'' }}</el-button>
...
export default {
  ...
  methods: {
    ...
    contentPageStyleChange(){
      ...
      this.$storage.set('crossTable','yuyue');
      this.$nextTick(() => {
